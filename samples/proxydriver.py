...
class ProxyTest(object):
    ...
    def header_test(self, hash_body=False):
        ...
        for i, (path, headers) in enumerate([
                ('/basic-cacheable?printheaders', []),
                ('/basic-cacheable?printheaders', ['x-forwarded-for: 10.11.12.13',]),
                ('/basic-uncacheable?printheaders', [])]):
            result = self.do_request(self.server_host + path, headers=headers)
            print("********** Request %d **********" % i)
            result.print_result(hash_body)
...
def main():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "v", ["port=", "test=", "proxy=", "cwd=", ])
